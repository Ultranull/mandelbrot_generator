...
void FrameBuffer::ResizeTexture(std::string name, GLuint w, GLuint h) {
	glBindFramebuffer(GL_FRAMEBUFFER, id);
	Texture text = textures[name];
	text.bind();
	text.setSize(w, h);
	glTexImage2D(GL_TEXTURE_2D, 0, text.params.internalformat, w, h, 0, text.params.format, GL_FLOAT, 0);
}
